...
async function getDataFromApi() {
    ...
    function DinamicCards(array) {
        ...
        <div  class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-4 col-xxl-3">
            <div class="card w-100 card-border mb-5  h-card">
                ...
                <a href="/details.html?id=${array[i]._id}" class="btn btn-primary">View more</a>
                </div>
            </div>
        </div>`;
            }
        } else {
            contenedorCard.innerHTML = `<p class="display-4"><i class="bi bi-calendar-x"></i> No results found. Try again</p></div>`
    ...
    const categoryCheckbox = arrayFuture.map(eventos => eventos.category) //puedo ponerle cualquier nombre a los parametros no necesariemnte "evento", podria llamarlo pepito y funcionaria igual.
    ...
    checkSelected.forEach(check => check.addEventListener("change", (evento => {
        ...
        if (isChecked) { //si esta checkeado los empujo y guardo en la variable
            ...
            //ACA TIENE QUE IR LA FUNCION DEL FILTRADO
            ...
            console.log(organizerCheck)
            //ACA TENDRIA QUE IR LA MISMA FUNCION DE FILTRADO
    ...
    function DisplayFilter() {
        ...
        keepChecks = []
        if (organizerCheck.length > 0 && textSearch === "") { //TIENE CHECK NO BUSQUEDA condicion para que se ejecute el if
            //recorremos la variable y mapeamos
            organizerCheck.map(recorrer => {
                keepChecks.push(...arrayFuture.filter(filtrado => filtrado.category === recorrer)) //Hacemos un push de las categorias en la variable, pero filtrando las repetidas 
            })
        } else if (organizerCheck.length === 0 && textSearch !== "") { //NO TIENE CHECK TIENE BUSQUEDA
            keepChecks.push(...arrayFuture.filter(e => e.name.toLowerCase().includes(textSearch.trim().toLowerCase())))

            //Esto se podria escribir solo con el else sin ninguna condicion
        } else if (organizerCheck.length > 0 && textSearch !== "") { //TIENE CHECK Y BUSQUEDA
            organizerCheck.map(recorrer => {
                keepChecks.push(...arrayFuture
                    .filter(filtrado => filtrado.category === recorrer)
                    .filter(e => e.name.toLowerCase().includes(textSearch)))
            })
        } else { //NO TIENE NINGUNO
            keepChecks.push(...arrayFuture) //Sino aplica el filtro, impreme todas las cards con la info del dataArray
        }
        console.log(keepChecks)
