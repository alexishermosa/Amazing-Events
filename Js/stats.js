/**Por las dudas, dejo acá lo que yo tengo entendido sobre los datos de la tabla. Si hay un error, corríjanme:

Events stadistics:
**-Evento con el mayor porcentaje de asistencia (solo entre los pasados)
**-Evento con el menor porcentaje de asistencia (solo entre los pasados)
**-Evento con mayor capacidad (usar eventos totales)
**Upcoming events stadistics by category
**Hay que agrupar todos los eventos futuros de la misma categoría y sacar el porcentaje de asistencia total estimado y un promedio de la ganancia estimada. Entonces va un solo dato por cada categoría.
**Past events stadistics by category
**Exactamente lo mismo que en upcomings pero usando la lista de Eventos Pasados
 */
...
async function getDataFromApi() {
  ...
  function PrintTableOne() {
    let contenedorUno = `<td >${MAX.eventos}: ${MAX.porAssistance}%</td>
  ...
  function PrintTableTwo() {
    ...
    estimateAndCapacityFuture = estimateAndCapacityFuture.sort((a, b) => b.estimateRevenue - a.estimateRevenue)
    estimateAndCapacityFuture.forEach(e => {
      e.estimateAndCapacityFuture
      contenedorDos += `<tr>
     <td  >${e.category}</td>
     <td >$${(new Intl.NumberFormat('de-DE').format(e.estimateRevenue))}</td>
  ...
  function PrintTableThree() {
    ...
    assistAndCapacityPast = assistAndCapacityPast.sort((a, b) => b.revenue - a.revenue)
