...
async function getDataFromApi() {
    ...
    function DinamicCards(array) {
        ...
        <div  class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-4 col-xxl-3">
            ...
            contenedorCard.innerHTML = `<div class="col-12 col-sm-12 col-md-6 col-lg-4 col-xl-4 col-xxl-3 d-flex justify-content-center"></div> <p class="display-4"><i class="bi bi-calendar-x"></i> No results found. Try again</p></div>`
    ...
    const categoryCheckbox = dataArray.map(eventos => eventos.category) //puedo ponerle cualquier nombre a los parametros no necesariemnte "evento", podria llamarlo pepito y funcionaria igual.
    ...
    function DisplayFilter() {
        ...
        if (organizerCheck.length > 0 && textSearch === "") { //condicion para que se ejecute el if
            ...
            organizerCheck.map(recorrer => {
                keepChecks.push(...dataArray.filter(filtrado => filtrado.category === recorrer)) //Hacemos un push de las categorias en la variable, pero filtrando las repetidas 
            })
        } else if (organizerCheck.length === 0 && textSearch !== "") {
            keepChecks.push(...dataArray.filter(e => e.name.toLowerCase().startsWith(textSearch.trim().toLowerCase())))
            //Esto se podria escribir solo con el else sin ninguna condicion
        } else if (organizerCheck.length > 0 && textSearch !== "") {
            organizerCheck.map(recorrer => {
                keepChecks.push(...dataArray
                    .filter(filtrado => filtrado.category === recorrer)
                    .filter(e => e.name.toLowerCase().startsWith(textSearch)))
            })
        } else {
            keepChecks.push(...dataArray) //Sino aplica el filtro, impreme todas las cards con la info del dataArray
        }
    ...
    // el crear id, arriba de todo, arreglo. map (e => e evnto.id sea id ++ ) // Crear ID
    // let dataEventos = data.eventos;
